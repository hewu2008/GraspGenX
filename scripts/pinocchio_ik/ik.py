"""Standalone pinocchio IK stack (no end2end_pipeline dependency).
...
reachable.  Also implements the SDK<->URDF base-frame calibration (``S_T_B``)
and the FK helper used by the LOW_LEVEL replays.

This package is intentionally independent of ``end2end_pipeline`` (mirroring
``curobo_planning``), so the low-level replay backends can run without pulling
in the high-level pipeline stack.  ``end2end_pipeline.ik_feasibility`` re-exports
...
from pinocchio_ik.logging_utils import get_logger
...
# project root = two levels up from this file (scripts/pinocchio_ik/...).
...
DEFAULT_RESIDUAL_TOL = 0.07     # 70 mm / 70 mrad IK convergence tolerance
...
def get_s_t_b(robot, side):
    """Compute the SDK<->body base transform ``S_T_B`` live from the robot.

    Unlike a cached calibration, ``S_T_B`` is re-measured on every call from the
    robot's *current* (static) arm pose via :func:`measure_s_t_b`, so
    it always reflects the motor-zero frame the robot is actually using.  The
    arm must be stationary while reading; returns None if any read/FK fails.
    """
    s_t_b = measure_s_t_b(robot, side)[0]
    if s_t_b is None:
        logger.error(f"[IK] Could not compute S_T_B live for {side}.")
    return s_t_b


def verify_fk_against_sdk(robot, side=None, samples=3, settle_s=0.3):
    ...
    differ only by a constant base transform; a large spread indicates per-joint
    zero / axis mismatch that a single base transform cannot fix.  This is a pure
    diagnostic (no state is cached): callers that need ``S_T_B`` obtain it live
    from :func:`get_s_t_b`.
